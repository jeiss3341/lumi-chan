...
module.exports = {
  ...
  README: {
    ...
    description: [
      ...
      '> • This bot also runs **Coastal Clash** (separate from bounties) — see `/readdeployment`.',
    ],
  },

  READDEPLOYMENT: {
    // index.js → the /readdeployment command's embed.
    title: '📋 Every /deploy Command',
    description: [
      'Run each of these once, wherever you want that system to live — they set up channels/categories and (for the Coastal Clash ones) auto-update from then on. Re-run any of them to move a board or change its settings.',
      '',
      '**💰 Bounty System**',
      '',
      '**`/deployrequestbounty`**',
      '> Sets up bounty **requests** and posts the request board. Options: **category** *(required)* — where new request tickets get created; **board** *(required)* — the channel the public request board posts to; **staff_user** / **staff_role** *(optional)* — who reviews requests and gets pinged.',
      '',
      '**`/deployclaimbounty`**',
      '> Sets up bounty **claiming** (both one-shot Claim bounties and ongoing Submissions/leaderboard bounties) and posts the claim board. Options: **claim_category** / **submissions_category** *(both required)* — separate ticket categories for each claim type; **board** — the finalized-claims log; **submissions_board** — the live, edited-in-place leaderboard board; **archive_category** *(required)* — where closed claim tickets go; **staff_user** / **staff_role** *(optional)*.',
      '',
      '**`/deployticket`**',
      '> Sets up general "talk to staff" support tickets (separate from bounty tickets). Options: **category** *(required)*; **archive_category** *(optional — omit and tickets just delete on close instead)*; **staff_user** / **staff_role** *(optional)*.',
      '',
      '**`/deployqanda`**',
      '> Posts the Q&A board in this channel. No options — it never creates tickets or pings staff, just answers common questions from a dropdown.',
      '',
      '**🌊 Coastal Clash**',
      '',
      ...
      '',
      '⚠️ **The Coastal Clash board settings are global, not per-server.** Running one of those three in a test server points that board there instead of the real one, until it\'s deployed again in the right channel.',
      '',
      '🗑️ **Deleting a Coastal Clash board message tells the bot to stop** — it won\'t auto-recreate itself there. Just re-run the matching `/deploy...` command wherever you want it back.',
    ],
  },
