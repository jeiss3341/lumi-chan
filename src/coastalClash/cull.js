...
// startedAt = null writes only next_cull_at and leaves last_updated_at
// alone (see db.setLeaderboardMeta) — pass null whenever RP wasn't actually
...
// True only if this cycle genuinely pulled RP from the ER API. A skipped
// refresh (season_live paused) or a failed season verification both leave
// player RP untouched, so neither may advance last_updated_at.
function didRefreshRP(refreshResult, seasonVerificationFailed) {
  return !seasonVerificationFailed && !refreshResult.skipped && refreshResult.updated > 0;
}
...
async function runDailyCull(now = new Date(), dryRun = false) {
  ...
  if (seasonVerificationFailed) {
    ...
    // logDiagnosticNicknameSweep (full-roster coverage during an outage)
    // was reverted here — during a sustained outage it adds real call
    // volume on every single cycle for as long as the outage lasts, which
    // is the wrong direction once the outage is suspected to be
    // volume/abuse-triggered rather than a simple rate limit. Reference-
    // player calls during season verification are still logged normally
    // (that's calls the pipeline makes regardless), so error visibility
    // isn't lost — just no longer expanded to the full roster.
  } else {
  ...
  if (!dryRun) await updateLeaderboardMeta(day, didRefreshRP(refreshResult, seasonVerificationFailed) ? now : null);
...
async function refreshLeaderboardOnly(now = new Date(), dryRun = false) {
  ...
  console.log(`[CC refresh ${process.env.LUMI_INSTANCE_ID}] start`);
  ...
  const seasonLive = (await db.getSeasonLive()) === 'true';
  console.log(`[CC refresh ${process.env.LUMI_INSTANCE_ID}] seasonLive =`, seasonLive);
  let seasonId = null;
  let corrected = false;
  let seasonVerificationFailed = false;
  if (seasonLive) {
    const referenceNicknames = await pickReferenceNicknames(db.pool);
    console.log(`[CC refresh ${process.env.LUMI_INSTANCE_ID}] got reference nicknames, verifying season...`);
    ({ seasonId, corrected } = await erApi.getVerifiedSeasonId(db, referenceNicknames, dryRun));
    console.log(`[CC refresh ${process.env.LUMI_INSTANCE_ID}] season verified:`, seasonId, 'corrected:', corrected);
    if (seasonId === null) seasonVerificationFailed = true;
  }
  ...
  if (seasonVerificationFailed) {
    refreshResult = { updated: 0, failed: [], skipped: true, reason: 'could not verify a live ER season ID this cycle (external API issue?) — RP fetch skipped, will retry next cycle' };
    // logDiagnosticNicknameSweep reverted — see the matching comment in
    // runDailyCull above for why.
  } else {
    refreshResult = await refreshAllRP(db.pool, seasonId, dryRun);
  }
  console.log(`[CC refresh ${process.env.LUMI_INSTANCE_ID}] RP refresh done:`, JSON.stringify({ updated: refreshResult.updated, failed: refreshResult.failed?.length }));
  ...
  console.log(`[CC refresh ${process.env.LUMI_INSTANCE_ID}] indanger updated for both brackets`);
  // Only stamp last_updated_at when RP actually got refreshed this cycle —
  ...
  const rpRefreshed = didRefreshRP(refreshResult, seasonVerificationFailed);
  if (!dryRun) await updateLeaderboardMeta(day, rpRefreshed ? now : null);
  console.log(`[CC refresh ${process.env.LUMI_INSTANCE_ID}] leaderboard_meta written — last_updated_at advanced:`, rpRefreshed);
  if (!dryRun) await db.pruneApiCallLog();
